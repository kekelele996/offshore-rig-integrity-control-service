...
func NewElectricalAssessment(asset, window string, zones []string, signals map[string]float64) ElectricalAssessment {
    ...
    return ElectricalAssessment{AssetID: strings.TrimSpace(asset), Window: strings.TrimSpace(window), Zones: copied, Signals: signalCopy}
}

func (a ElectricalAssessment) NormalizeZones() []string {
...
func (a ElectricalAssessment) Signal(name string) float64 { return a.Signals[strings.ToLower(strings.TrimSpace(name))] }
func (a *ElectricalAssessment) AddNote(note string) { if strings.TrimSpace(note) != "" { a.Notes = append(a.Notes, strings.TrimSpace(note)) } }
...
func (a ElectricalAssessment) Result() ElectricalResult {
    ...
    return ElectricalResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeZones()}
}

func (a ElectricalAssessment) Merge(other ElectricalAssessment) ElectricalAssessment {
    out := NewElectricalAssessment(a.AssetID, a.Window, a.Zones, a.Signals)
...
func (a ElectricalAssessment) RequiresSupervisor() bool { r := a.Result(); return r.Review || r.Band == "high" }
func (a ElectricalAssessment) StableKey() string { r := a.Result(); return strings.Join([]string{a.AssetID, a.Window, strings.Join(r.Normalized, ",")}, "#") }
