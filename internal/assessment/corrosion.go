...
func NewCorrosionAssessment(asset, window string, zones []string, signals map[string]float64) CorrosionAssessment {
    ...
    return CorrosionAssessment{AssetID: strings.TrimSpace(asset), Window: strings.TrimSpace(window), Zones: copied, Signals: signalCopy}
}

func (a CorrosionAssessment) NormalizeZones() []string {
...
func (a CorrosionAssessment) Signal(name string) float64 { return a.Signals[strings.ToLower(strings.TrimSpace(name))] }
func (a *CorrosionAssessment) AddNote(note string) { if strings.TrimSpace(note) != "" { a.Notes = append(a.Notes, strings.TrimSpace(note)) } }
...
func (a CorrosionAssessment) Result() CorrosionResult {
    ...
    return CorrosionResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeZones()}
}

func (a CorrosionAssessment) Merge(other CorrosionAssessment) CorrosionAssessment {
    out := NewCorrosionAssessment(a.AssetID, a.Window, a.Zones, a.Signals)
...
func (a CorrosionAssessment) RequiresSupervisor() bool { r := a.Result(); return r.Review || r.Band == "high" }
func (a CorrosionAssessment) StableKey() string { r := a.Result(); return strings.Join([]string{a.AssetID, a.Window, strings.Join(r.Normalized, ",")}, "#") }
