...
func (a EvidenceAssessment) Signal(name string) float64 {
	return a.Signals[strings.ToLower(strings.TrimSpace(name))]
}
func (a *EvidenceAssessment) AddNote(note string) {
...
func (a EvidenceAssessment) RequiresSupervisor() bool {
	r := a.Result()
	return r.Review || r.Band == "high"
}
func (a EvidenceAssessment) StableKey() string {
	r := a.Result()
