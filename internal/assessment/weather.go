...
func NewWeatherAssessment(asset, window string, zones []string, signals map[string]float64) WeatherAssessment {
    ...
    return WeatherAssessment{AssetID: strings.TrimSpace(asset), Window: strings.TrimSpace(window), Zones: copied, Signals: signalCopy}
}

func (a WeatherAssessment) NormalizeZones() []string {
...
func (a WeatherAssessment) Signal(name string) float64 { return a.Signals[strings.ToLower(strings.TrimSpace(name))] }
func (a *WeatherAssessment) AddNote(note string) { if strings.TrimSpace(note) != "" { a.Notes = append(a.Notes, strings.TrimSpace(note)) } }
...
func (a WeatherAssessment) Result() WeatherResult {
    ...
    return WeatherResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeZones()}
}

func (a WeatherAssessment) Merge(other WeatherAssessment) WeatherAssessment {
    out := NewWeatherAssessment(a.AssetID, a.Window, a.Zones, a.Signals)
...
func (a WeatherAssessment) RequiresSupervisor() bool { r := a.Result(); return r.Review || r.Band == "high" }
func (a WeatherAssessment) StableKey() string { r := a.Result(); return strings.Join([]string{a.AssetID, a.Window, strings.Join(r.Normalized, ",")}, "#") }
