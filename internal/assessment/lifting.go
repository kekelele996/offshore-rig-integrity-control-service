...
func NewLiftingAssessment(asset, window string, zones []string, signals map[string]float64) LiftingAssessment {
    ...
    return LiftingAssessment{AssetID: strings.TrimSpace(asset), Window: strings.TrimSpace(window), Zones: copied, Signals: signalCopy}
}

func (a LiftingAssessment) NormalizeZones() []string {
...
func (a LiftingAssessment) Signal(name string) float64 { return a.Signals[strings.ToLower(strings.TrimSpace(name))] }
func (a *LiftingAssessment) AddNote(note string) { if strings.TrimSpace(note) != "" { a.Notes = append(a.Notes, strings.TrimSpace(note)) } }
...
func (a LiftingAssessment) Result() LiftingResult {
    ...
    return LiftingResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeZones()}
}

func (a LiftingAssessment) Merge(other LiftingAssessment) LiftingAssessment {
    out := NewLiftingAssessment(a.AssetID, a.Window, a.Zones, a.Signals)
...
func (a LiftingAssessment) RequiresSupervisor() bool { r := a.Result(); return r.Review || r.Band == "high" }
func (a LiftingAssessment) StableKey() string { r := a.Result(); return strings.Join([]string{a.AssetID, a.Window, strings.Join(r.Normalized, ",")}, "#") }
