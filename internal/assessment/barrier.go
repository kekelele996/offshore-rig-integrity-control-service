...
func (a BarrierAssessment) Signal(name string) float64 {
	return a.Signals[strings.ToLower(strings.TrimSpace(name))]
}
func (a *BarrierAssessment) AddNote(note string) {
...
func (a BarrierAssessment) RequiresSupervisor() bool {
	r := a.Result()
	return r.Review || r.Band == "high"
}
func (a BarrierAssessment) StableKey() string {
	r := a.Result()
