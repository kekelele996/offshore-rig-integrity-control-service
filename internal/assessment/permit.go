...
func (a PermitAssessment) Signal(name string) float64 { return a.Signals[strings.ToLower(strings.TrimSpace(name))] }
func (a *PermitAssessment) AddNote(note string) { if strings.TrimSpace(note) != "" { a.Notes = append(a.Notes, strings.TrimSpace(note)) } }
...
func (a PermitAssessment) RequiresSupervisor() bool { r := a.Result(); return r.Review || r.Band == "high" }
func (a PermitAssessment) StableKey() string { r := a.Result(); return strings.Join([]string{a.AssetID, a.Window, strings.Join(r.Normalized, ",")}, "#") }
